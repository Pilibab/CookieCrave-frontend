...
import type { Order, Product, CartOrderLineItem } from "@/types";
        ...
        async function loadCustomerOrders() {
            try {
                ...
                allProducts.forEach((p) =>
                    productMap.set(p.prod_id, p.prod_name),
                );
                ...
                const enrichedOrders: EnrichedOrder[] = await Promise.all(
                    customerOrders.map(async (order) => {
                        try {
                            const lineItems: CartOrderLineItem[] =
                                await cartApi.getByOrder(order.ord_id);

                            // Map line items to strings like "2x Cinnamon Cookie"
                            const namesArray = lineItems.map((item) => {
                                const prodName =
                                    productMap.get(item.prod_id) ||
                                    "Unknown Cookie";
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    Real-time status and fulfillment logs for all your cookie
                    purchases.
                </p>
